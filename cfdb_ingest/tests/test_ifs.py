...
# ---------------------------------------------------------------- fail-loudly guards (0.4.2, ahead of IFS Cycle 50r2)


def test_units_guard_refuses_a_source_in_other_units(tmp_path, ifs_cycle_dir, monkeypatch):
    """50r2's sample carries sd/tp in kg m-2 under the same shortNames; a units change must not be mis-scaled."""
    from cfdb_ingest import ifs as ifs_mod

    monkeypatch.setitem(ifs_mod.IFS_SOURCE_UNITS, 'sd', 'kg m**-2')  # the file says 'm of water equivalent'
    with pytest.raises(ValueError, match="sd is in 'm of water equivalent', the mapping expects 'kg m\\*\\*-2'"):
        IfsIngest(ifs_cycle_dir)
    # every source var of the real 0.25 deg open data (as generated by ifs_synthetic) is in the table
    assert {sv for info in IFS_VARIABLE_MAPPING.values() for sv in info['source_vars']} <= set(ifs_mod.IFS_SOURCE_UNITS)


def test_duplicate_message_is_refused(tmp_path, ifs_cycle_dir):
    """The same (field, level, step) twice -- e.g. a field disseminated in two encodings -- never last-one-wins."""
    src = sorted(ifs_cycle_dir.glob('*.grib2'))[0]
    dup = tmp_path / 'dup.grib2'
    dup.write_bytes(src.read_bytes() + src.read_bytes())
    with pytest.raises(ValueError, match='appears twice'):
        IfsIngest([dup])


def test_unknown_level_type_is_refused_for_a_mapped_source(tmp_path, ifs_cycle_dir, monkeypatch):
    """A mapped source on a level type the ingest cannot place (50r2's snowLayer) is an error, not 'surface'."""
    from cfdb_ingest import ifs as ifs_mod

    monkeypatch.setattr(ifs_mod, '_SURFACE_LEVEL_TYPES', ('surface',))  # 2t is on heightAboveGround
    with pytest.raises(ValueError, match="2t is on level type 'heightAboveGround'"):
        IfsIngest(ifs_cycle_dir)
    # ... but an UNMAPPED field on an unknown level type is just ignored (the gap-filler messages)
    monkeypatch.setattr(
        ifs_mod, '_SURFACE_LEVEL_TYPES', ('surface', 'heightAboveGround', 'entireAtmosphere', 'mostUnstableParcel')
    )  # everything but meanSea
